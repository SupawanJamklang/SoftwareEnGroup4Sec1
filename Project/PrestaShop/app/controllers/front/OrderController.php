...
class OrderControllerCore extends FrontController
{
    ...
    protected function buildLastCheckoutProcess(CheckoutSession $session, $translator)
    {
        $checkoutProcess = new CheckoutProcess(
            $this->context,
            $session
    ...
    public function displayExpressCheckoutWithoutInvoice()
{
    // ทำขั้นตอนที่ต้องการเมื่อคลิก "Proceed Without Invoice"
    // ตัวอย่าง: ไปที่หน้าขอบคุณ
    $this->context->smarty->assign([
        'template_path' => _PS_THEME_DIR_ . 'templates/checkout/payment-success.tpl',
    ]);

    $this->setTemplate('checkout/payment-success.tpl');
}
